...
static void parent_send(mqd_t request) {
    ...
    // Answer 1: Send input to the child with mq_send().
    mq_send(request, input, sizeof(input), 0);
}
...
int main(void) {
    ...
    if (pid == 0) {
        ...
        printf("Child sum: %d\n", sum);
        // Answer 2: Send sum to the parent with mq_send().
        mq_send(response, &sum, sizeof(sum), 0);
